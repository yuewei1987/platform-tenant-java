...
public interface UserDao extends JpaRepository<User, String>, JpaSpecificationExecutor<User> {
    ...
    @Query("from User where (account=?1 or phone=?1) and delFlag='0'")
    User findUser(String account);

    @Query("from User where (account=?1 or phone=?1) and delFlag='0' and userId <> ?2")
    User findUser(String account, String userId);
